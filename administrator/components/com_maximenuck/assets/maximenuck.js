...
function load_gfont_stylesheets() {
	var gfonturl1 = get_gfont_stylesheet($ck('#menustylestextgfont').val());
	var gfonturl2 = get_gfont_stylesheet($ck('#level2itemnormalstylestextgfont').val());
...
function save_param(id, param, value) {
	...
	$ck.ajax({
		...
		window.parent.$ck('#jform_params_'+param).val(value);
	}).fail(function() {
		alert(Joomla.JText._('CK_FAILED', 'Failed'));
	});
}
...
function save_stylesparam(id, param, layout) {
	...
	$ck.ajax({
		...
		if (layout == 'modal') {
			window.parent.$ck('#jform_params_'+param).val(param_value);
		}
	}).fail(function() {
		alert(Joomla.JText._('CK_FAILED', 'Failed'));
	});
