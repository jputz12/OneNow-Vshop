<?php
/**
 * @version $Id: items.php 373 2015-02-10 08:41:53Z michal $
 * @package DJ-Catalog2
...
class Djcatalog2ControllerItems extends JControllerAdmin
{
    ...
    function batch_approval()
	{
		...
		// Get items to publish from the request.
		$producer	= $app->input->get('filter_producer');
		$task 	= $this->getTask();
		$value = ($task == 'batch_approval') ? 1 : 0;
	
		if (empty($producer)) {
			JError::raiseWarning(500, 'No producer selected.');
		}
		else
        {
            $approved = 0;
            $deleted = 0;
            $db->setQuery("UPDATE #__djc2_items SET published = '1', approval_status = '' WHERE approval_status = 'for approval' AND producer_id = '".$producer."'");
            $res = $db->execute();
            $approved = $db->getAffectedRows($res);
            
            $db->setQuery("UPDATE #__djc2_items SET published = '-2', approval_status = '' WHERE approval_status = 'for deletion' AND producer_id = '".$producer."'");
            $res = $db->execute();
            $deleted = $db->getAffectedRows($res);
            
            if($approved == 0 && $deleted == 0)
            {
                JError::raiseWarning(500, 'No products for batch approval.');
            }
            else
            {
                $this->setMessage('Batch approval done. There are '.$approved.' approvals and '.$deleted. ' deletions.');
            }
        }
		$extension = $app->input->get('extension', null, 'cmd');
		$extensionURL = ($extension) ? '&extension=' . $app->input->get('extension', null, 'cmd') : '';
