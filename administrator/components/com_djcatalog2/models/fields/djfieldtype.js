...
var Djfieldtype = new Class(
{
			...
			*/
			...
			document.id(this.typeSelector).onchange = (function(evt) {
				...
				if (typeof(jQuery) != 'undefined') {
					...
					jQuery('#jform_filter_type').trigger("liszt:updated");
				}
			}.bind(this));
		}
	},
	displayForm : function() {
		...
		var switch_ft = document.id('jform_filter_type');
		...
		if (!this.fieldtype || this.fieldtype =='empty') {
			...
			if (switch_ft) {
				switch_ft.value='0';
				switch_ft.setAttribute('disabled','disabled');
			}
			if (switch_s) {
				switch_s.value='0';
			...
			if (this.fieldtype == 'calendar') {
			...
			else if (this.fieldtype != 'select' && this.fieldtype != 'checkbox' && this.fieldtype != 'radio' && this.fieldtype != 'text') {
				if (switch_f) {
					switch_f.value='0';
					switch_f.setAttribute('disabled','disabled');
				}
				...
				switch_s.removeAttribute('disabled');
				switch_f.removeAttribute('disabled');
				switch_ft.removeAttribute('disabled');
