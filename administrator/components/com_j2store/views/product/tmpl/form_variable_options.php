		...
		$(document).ready(function() {
			$('#optionselector').autocomplete({
				...
				select : function(event, ui) {
					$('<tr><td class=\"addedOption\">' + ui.item.label+ '</td><td><span class=\"optionRemove\" onclick=\"j2store.jQuery(this).parent().parent().remove();\">x</span><input type=\"hidden\" value=\"' + ui.item.value+ '\" name=\"<?php echo $this->form_prefix; ?>[item_options]['+ ui.item.value+'][option_id]\" /><input type=\"hidden\" value="" name=\"<?php echo $this->form_prefix; ?>[item_options]['+ ui.item.value+'][j2store_productoption_id]\" /></td></tr>').insertBefore('.j2store_a_options');
					this.value = '';
					return false;
				},
...
function removePAOption(pao_id) {
	...
	$.ajax({
			type : 'post',
			url :  'index.php?option=com_j2store&view=products&task=removeProductOption&product_type=variable',
			data : 'pao_id=' + pao_id,
			dataType : 'json',
			success : function(data) {
				console.log(data);
