		...
		<div class="row-fluid">
				...
				<div class="product-upsells">
					...
					<table class="table table-striped table-bordered">
						...
						<?php
							...
							$upsells  = explode(',',$this->item->up_sells); ?>
						...
						<?php if(isset($related_product->product_source_id)):?>
							<tr id="upSell-<?php echo $value;?>">
									<td class="addedProductUpsell">
										...
										<input type="hidden" value="<?php echo $value;?>"  name="<?php echo $this->form_prefix.'[up_sells]' ;?>[<?php echo $value;?>]" />
									</td>
									<td>
										<a href="javascript:void(0);" onclick="removeThisRelatedRow('upSell',<?php echo $value;?>)">
			...
			<div class="product-crosssells">
						...
						<table class="table table-striped table-bordered">
							...
							$crosssells  = explode(',',$this->item->cross_sells); ?>
							<?php foreach($crosssells as $key=>$value):?>
									<?php
							//$relatedModel = F0FModel::getAnInstance('Products', 'J2StoreModel')->getClone();
							//$related_product = $relatedModel->getItem($value);
							$product = J2Store::product();
							...
							?>
								<?php if(isset($related_product->product_source_id)):?>
								<tr id="crossSell-<?php echo $value;?>">
										<td class="addedProductCrosssell">
											<?php
											$related_product= J2Store::product()
												->clearState()
												->product_id($value)
												->getProduct();
											?>
											...
											<input type="hidden" value="<?php echo $value;?>" name="<?php echo $this->form_prefix.'[cross_sells]' ;?>[<?php echo $value;?>]" />
										</td>
										<td>
											<a href="javascript:void(0);" onclick="removeThisRelatedRow('crossSell',<?php echo $value;?>)">
												<i class="icon icon-trash"></i>
											</a>
										</td>
									</tr>
	...
	$(document).ready(function() {
		$('#upsellSelector').autocomplete({
			source : function(request, response) {
				$.ajax({
					type : 'post',
					url :  'index.php?option=com_j2store&view=products&task=getRelatedProducts&product_id=<?php echo $this->item->j2store_product_id;?>',
					data : 'q=' + request.term,
					dataType : 'json',
