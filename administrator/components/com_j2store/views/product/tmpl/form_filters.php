...
<script type="text/javascript">
						...
						function removeFilter(filter_id, product_id) {
							(function($) {
								...
								$.ajax({
									type : 'post',
									url :  'index.php?option=com_j2store&view=products&task=deleteproductfilter',
									data : {filter_id:filter_id, product_id:product_id},
									dataType : 'json',
		...
		$(document).ready(function() {
			$('#J2StoreproductFilter').autocomplete({
				source : function(request, response) {
					$.ajax({
						type : 'post',
						url :  'index.php?option=com_j2store&view=products&task=searchproductfilters',
						data : 'q=' + request.term,
						dataType : 'json',
						success : function(data) {
