					...
					<tr>
							...
							<?php
								$thumbimage = '';
							 	...
							 	<a target="_blank" href="<?php echo $item->product_edit_url;?>">
								 	<img class="j2store-product-thumb-image"
								...
								<br />
								<span>
									<?php echo JText::_('J2STORE_PRODUCT_TYPE')?> : <label class="label label-info"><?php echo $item->product_type; ?></label>
								</span>
								<br />
								<span>
									<?php echo JText::_('J2STORE_PRODUCT_VISIBILITY')?> : <label class="label label-<?php echo $item->visibility ? 'success':'important'; ?>"><?php echo $item->visibility ? JText::_('JYES'):JText::_('JNO'); ?></label>
								</span>
						...
						<td colspan="4">
							...
							<button type="button" class="btn btn-small btn-warning" id="showvariantbtn-<?php echo $item->j2store_product_id;?>" href="javascript:void(0);" onclick="jQuery('#hide-icon-<?php echo $item->j2store_product_id;?>').toggle('click');jQuery('#show-icon-<?php echo $item->j2store_product_id;?>').toggle('click');jQuery('#variantListTable-<?php echo $item->j2store_product_id;?>').toggle('click');" >
								...
								<i id="show-icon-<?php echo $item->j2store_product_id;?>" class="icon icon-plus"></i>
								<i id="hide-icon-<?php echo $item->j2store_product_id;?>" class="icon icon-minus" style="display:none;"></i>
							</button>
						...
						<td><?php echo  $item->product_source;?></td>
