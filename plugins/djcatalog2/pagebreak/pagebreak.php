<?php
/**
 * @version $Id: pagebreak.php 372 2015-02-04 06:46:47Z michal $
 * @package DJ-Catalog2
	...
	function onPrepareItemDescription( &$row, &$params, $page=0, $context = 'item' )
	{
		...
		if ($n > 1)
		{
			...
			if ($style == '1') {
				...
				for($i = 1; $i < $n; $i++) {
					$id = strtolower(preg_replace('#[^0-9a-zA-Z\-]#i', '_', $title[$i-1]));
					
				    $class = ($i == 1) ? 'class="accordion-body collapse in"' : 'class="accordion-body collapse"';
				    $row->tabs .= '<div class="accordion-group">';
					$row->tabs .= '<div class="accordion-heading"  id="'.$id.'"><a class="accordion-toggle" href="#'.$id.'">'.$title[$i-1].'</a></div>';
				...
				for($i = 1; $i < $n; $i++) {
					...
					$row->tabs .= '<li '.$class.' id="'.$id.'"><a href="#'.$id.'">'.$title[$i-1].'</a></li>';
				}
				$row->tabs .='</ul>';
