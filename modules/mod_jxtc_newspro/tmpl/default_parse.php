...
//Get tags
if ($support_tags && stripos($itemhtml,'{tags}') !== false) {
	$aux = new stdClass();
...
$itemhtml = str_replace( '{tags}', $tagsHTML, $itemhtml );
...
foreach ($tagmodel as $tag => $value) {
  ...
  if ($match) {
	...
	$itemhtml = str_replace('{'.$tag.'}',$value, $itemhtml);
}
//dates
$tagmodel = array(
	'date' => $item->created,
...
foreach ($tagmodel as $tag => $value) {
  ...
  if ($match) {
	...
	$itemhtml = str_replace('{'.$tag.'}', JHtml::_('date', $value, $dateformat), $itemhtml);
}
