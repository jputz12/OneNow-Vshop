...
foreach ($list as $index => &$item) :
	...
	if($first) {
		...
		echo '</ul></div><div class="dj-subcol" style="width:150px;"><ul class="dj-submenu">';
		$class .= ' first';
	}
	...
	if ($item->deeper) {
		if($item->level > $startLevel && $expand[$item->id]=='tree') {
			...
			echo '<div class="dj-subwrap '.$open_dir.' '.($subcols[$item->id] > 1 ? 'multiple_cols':'single_column').' subcols'.$subcols[$item->id].'"><div class="dj-subwrap-in" style="width:1170px">';
			...
			echo '<div class="dj-subcol" style="width:150px;"><ul class="dj-submenu">';
		}
		$first = true;
	}
